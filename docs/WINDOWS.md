...
| `electron-builder.yml:43` | der ganze `win:`-Block (`nsis` + `portable`, `x64`) plus die `nsis:`- und `portable:`-Optionen | Die einzige plattformabhängige Stelle außerhalb von `src/`: macOS bekommt DMG + ZIP, Windows einen NSIS-Installer und eine portable `.exe`. Kein Code, sondern Konfiguration. | Am 2026-08-20 gebaut, installiert und gestartet (Abschnitt 4a): `build/icon.ico` hat genug Auflösungen, der Installer nimmt ein wählbares Verzeichnis (`allowToChangeInstallationDirectory: true`) und läuft ohne Adminrechte (`perMachine: false`). Offen bleibt nur, wie sich der Installer bei einem **Update über eine bestehende Installation** verhält — bisher ist nur eine Neuinstallation gelaufen |
