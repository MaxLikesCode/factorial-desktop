...
> — 23 Treffer in `src/`, zwei in der YAML.
>
...
| `electron-builder.yml:35` | `verifyUpdateCodeSignature: false` | Nur Windows kennt diese Prüfung: electron-updater vergleicht die Signatur der heruntergeladenen Datei mit der der laufenden App, bevor es installiert. Diese Builds sind unsigniert, es gibt also nichts zu vergleichen — die Prüfung schlägt fehl und das Update wird stillschweigend nie angewandt. Deshalb ausdrücklich aus, statt es im letzten Schritt scheitern zu lassen. | Ein Update von einer Version zur nächsten wirklich durchlaufen lassen (Abschnitt 4b). Sobald es ein Zertifikat gibt, gehört die Zeile **weg** — sie ausgeschaltet zu lassen, obwohl signiert wird, gäbe die Schutzwirkung ohne Not auf |
| `electron-builder.yml:39` | der ganze `win:`-Block (`nsis` + `portable`, `x64`) plus die `nsis:`- und `portable:`-Optionen | Die einzige plattformabhängige Stelle außerhalb von `src/`: macOS bekommt DMG + ZIP, Windows einen NSIS-Installer und eine portable `.exe`. Kein Code, sondern Konfiguration. | Am 2026-08-20 gebaut, installiert und gestartet (Abschnitt 4a): `build/icon.ico` hat genug Auflösungen, der Installer nimmt ein wählbares Verzeichnis (`allowToChangeInstallationDirectory: true`) und läuft ohne Adminrechte (`perMachine: false`). Offen bleibt, wie sich der Installer bei einem **Update über eine bestehende Installation** verhält |
| `src/main/update-policy.ts:36` | `installKind` unterscheidet `portable` von `installed` | Den Fall gibt es nur auf Windows — für macOS existiert kein portables Target. Er ist wichtig, weil eine portable `.exe` sich **nicht** selbst ersetzen kann: sie entpackt sich bei jedem Start nach `%TEMP%` und läuft von dort, die Datei, die der Benutzer aufbewahrt, liegt woanders. Ein Update-Download dorthin würde nichts aktualisieren. Deshalb darf die portable Fassung prüfen, aber nicht installieren, und verweist stattdessen auf die Download-Seite. Die Funktion selbst ist plattformfrei geschrieben und nimmt ihre Eingaben als Argumente — nur so ist der Windows-Fall überhaupt von woanders testbar. | Portable starten, im Tray „Nach Updates suchen …": es muss der **Hinweis mit Download-Link** kommen, kein Download-Fortschritt. Beim Installer dagegen der echte Update-Ablauf |
...
vorhanden herausgestellt hat. Die Tabelle hat **25** Zeilen und deckt
`grep -rn "PLATFORM:" src/ electron-builder.yml` vollständig ab (23 Treffer in
acht Dateien unter `src/`, plus zwei in der YAML). Die Tasks 13
...
1. **Linksklick** auf das Icon blendet das Widget ein und wieder aus
   (`src/main/tray.ts:269`). **Rechtsklick** öffnet das Menü. **Doppelklick**
...
3. **Zweiter Start** (Verknüpfung doppelklicken, während die App läuft): es darf
   **keine** zweite Instanz und kein zweites Icon erscheinen, stattdessen kommt
   das Widget nach vorn (`src/main/index.ts:258` und `:261`).
4. **Widget schließen** → App läuft weiter, Icon bleibt. **„Beenden"** im Menü →
   der Prozess ist im Task-Manager wirklich weg (`src/main/index.ts:282`).

### Schritt 5 — Das Fenster

1. **Transparenz:** keine weißen Ecken hinter den runden Kanten, kein eckiger
   Schattenrahmen (`src/main/windows.ts:120`). Falls doch, sind `transparent`,
...
4. **Über Vollbild:** `visibleOnFullScreen` gibt es auf Windows nicht, dort
   trägt allein `alwaysOnTop` (`src/main/windows.ts:139`). Bleibt das Widget
